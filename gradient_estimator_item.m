...
%   a, b, c, D: items parameters
    ...
    for i=1:NumIterations
       ...
       result_a = [result_a; [a_hat log_likelihood(Y, theta, a_hat, b_hat, c_hat, D)]];
       a_hat = a_hat + alpha * derivative_a(Y, theta, a_hat, b_hat, c_hat, D);

       result_b = [result_b; [b_hat log_likelihood(Y, theta, a_hat, b_hat, c_hat, D)]];
       b_hat = b_hat + alpha * derivative_b(Y, theta, a_hat, b_hat, c_hat, D);

       result_c = [result_c; [c_hat log_likelihood(Y, theta, a_hat, b_hat,c_hat,D)]];
       c_hat = c_hat + alpha * derivative_c(Y, theta, a_hat, b_hat, c_hat, D);
