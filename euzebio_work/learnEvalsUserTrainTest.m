...
%             grupo2 = ind(1);
            ...
            for a=1:numel(areas)
                ...
                idQuestoesTrain = [idQuestoesTrain; randsample(questoesArea, round(3*numel(questoesArea)/4))];
            end
            
            idQuestoesTrain = sort(idQuestoesTrain);
            idQuestoesTest = idQuestoes(~ismember(idQuestoes, idQuestoesTrain));
            ...
            evalsPositivoTest = zeros(length(idQuestoes),length(idAlunos));
            evalsNegativoTest = zeros(length(idQuestoes),length(idAlunos));
            for i=1:size(evalsTest,1)
                u = find(idAlunos==evalsTest(i,1));
                q = find(idQuestoes==evalsTest(i,2));
                if evalsTest(i,3)
                    evalsPositivoTest(q,u) = 1;
                else
                    evalsNegativoTest(q,u) = 1;
                end
            end
    ...
    for Alat=valuesAlat
        thetaMax = sqrt(1e300/Alat);
            ...
            load(strcat('thetas-grupo-', num2str(grupo1), 'Alat-', num2str(Alat), '-turn-', num2str(turns), '.mat'), 'thetaObj');
            ...
            logF = max(log(f),-1e300);
            log1menosF = max(log(1-f),-1e300);
            erro = sum(sum(evalsPositivoTest.*logF)) + sum(sum(evalsNegativoTest.*log1menosF));  %nao inclui lambda
            
            auc = sum(sum(evalsPositivoTest.*(f>.5))) + sum(sum(evalsNegativoTest.*(f<=.5)));
            if isnan(erro)
                disp('debug 1')
                break;
            end
            ...
            save(strcat('test-users-grupo1-', num2str(grupo1),'-Alat-', num2str(Alat), '-turn-', num2str(turns),'-grupo2-', num2str(grupo2), '.mat'), 'Alat', 'grupo1', 'grupo2', 'thetaObj', 'thetaUser', 'auc', 'erro', 'idQuestoes', 'aucPlot', 'idQuestoesTrain');
        end
    end
end