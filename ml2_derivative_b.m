...
%   a, b, c, D: items parameters
    ...
    for i=1:I
        ...
        for j=1:N
            ...
            b_sum = b_sum + ( ((U(j, i) - ml2_Pji(theta_j, a_i, b_i, D)) / (ml2_Pji(theta_j, a_i, b_i, D).*ml2_Qji(theta_j, a_i, b_i, D)) ) .* ( (a_i .* D .* exp(L(theta_j,a_i,b_i,D))) / ((1 + exp(L(theta_j,a_i,b_i,D))).^2) ) );
        end
        b_hat(i) = b_sum;
    end
end
