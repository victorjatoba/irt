...
%   a, b, c, D: items parameters
    ...
    for i=1:I
        ...
        for j=1:N
            ...
            a_sum = a_sum + ( ((U(j, i) - ml2_Pji(theta_j, a_i, b_i, D)) / (ml2_Pji(theta_j, a_i, b_i, D).*ml2_Qji(theta_j, a_i, b_i, D)) ) .* -( (D*(theta_j-b_i) .* exp(L(theta_j,a_i,b_i,D))) / sqrt(1 + exp(L(theta_j,a_i,b_i,D))) ) );
        end
        a_hat(i) = a_sum;
    end
end
